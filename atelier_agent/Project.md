...
- **Name:** Atelier
- **Status:** All phases (0–7) shipped. Dual-mode local agent + memory + MCP + hybrid retrieval + eval harness + a LoRA-fine-tuned router + one-command repro + writeup. Eval suite expanded to 18 doc-QA + 13 code-repair tasks + 10 combined knowledge→build tasks, with SVG plot generation and recorded expanded results. Current live results: doc-QA 94% correct / 83% retrieval / 100% cited, code 100%, combined 100% on 10 tasks. Open follow-ups: train/evaluate the planner-router adapter, add harder multi-file combined tasks, public release.
- **Owner:** Monit
...
- 2026-06-22 — Eval expansion keeps prior baseline tasks and appends new frozen tasks with metadata (`category`, `difficulty`, `edit_scope`) — preserves comparability while enabling reliability curves by task family and repair shape.
- 2026-06-22 — Eval plots are stdlib-generated SVGs rather than a new plotting dependency — keeps reporting local, free, and reproducible while still producing shareable performance charts.
...
- 2026-06-22 — README retitled to "Project Atelier" and refreshed as the main public project guide. Eval expansion shipped: doc-QA grew from 8 to 18 tasks; code-repair grew from 3 to 13 tasks across arithmetic, off-by-one, normalization, mutation, order preservation, and structural-logic categories. `eval/run_eval.py` now carries metadata into reports and emits grouped aggregates by category/difficulty/edit scope. Added fast tests for metadata aggregation and report breakdowns.
- 2026-06-22 — Added `eval/plots.py`, `atelier eval-plots`, and `make eval-plots`. Saved eval reports can now produce SVG charts for doc-QA overview, doc-QA by category, code overview, code solved by difficulty/edit scope, and code steps by task. Plot generation is covered by `tests/test_eval_plots.py`.
- 2026-06-22 — Recorded expanded eval result in README/docs with checked-in SVG plots. Latest 18+13 run: doc-QA correctness 94%, citations 94%, retrieval hit 61%; code solved 92% overall, 100% for single-line edits, 80% for multi-line edits. Remaining failure: `median_bug`, confirming structural multi-line edits as the next reliability target.
- 2026-06-22 — Added `ast_edit`, a workspace-safe Python function-body replacement tool that compile-checks before writing. The ReAct prompt now tells the agent to prefer it for multi-line Python edits, directly targeting the `median_bug` failure mode. Doc-QA retrieval metrics now support multiple acceptable expected sources per task.
- 2026-06-22 — Fixed a repo navigation bug exposed by `median_bug`: `repo_map` now emits workspace-relative paths that can be copied directly into `read_file`/`edit_file`/`ast_edit`. Targeted live rerun of `median_bug` now passes in 5 steps with 0 tool errors; full code-suite rerun later passed 13/13.
- 2026-06-22 — Full code-suite rerun after the repo-map path fix passed: 13/13 solved, average 5.8 steps, 0.0 tool errors (`report_20260621T171954`). README, eval docs, plots, and `myNotes.md` updated with the before/intervention/after result.
- 2026-06-22 — Added combined knowledge→build eval suite (`eval/tasks_combined/`), now broadened to 10 tasks. Combined success requires both passing tests and `search_notes` in the trace. Added planner-router SFT dataset builder (`models/router/make_planner_dataset.py`) producing 41 rows from real eval metadata: task → category, difficulty, edit scope, tool plan, model route.
- 2026-06-22 — Live combined eval passed: 10/10 solved, 10/10 tests passed, 10/10 used `search_notes`, average 6.7 steps (`report_20260622T011056`). Fixed a mixed-indentation `ast_edit` normalization issue found by the first 10-task run, then copied combined plots into `docs/assets/eval/report_20260622T011056/`.
